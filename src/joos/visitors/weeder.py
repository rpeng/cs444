...
class WeederVisitor(object):
    ...
    def VisitMethodDecl(self, node):
        ...
        if ('abstract' in modifiers or 'native' in modifiers) and node.body != None:
        ...
        if 'native' in modifiers and not 'static' in modifiers:
    ...
    # def VisitClassMemberDecl(self, node):
        # modifiers = [x.lexeme for x in node.modifiers]
        # if node.cf_type == 'FieldDeclaration' and len(modifiers) == 0:
            # _err(node.decl, "Package cannot have private field") 
        # if node.cf_type == 'MethodDeclaration' and (
                # (not 'public' in modifiers) and
                # (not 'protected' in modifiers)):
            # _err(node.decl, "Package cannot have private method")
        # for child in node.children:
            # child.visit(self)
    ...
    def VisitFieldDecl(self, node):
        modifiers = [x.lexeme for x in node.modifiers]
        if 'final' in modifiers and node.var_decl == None:
