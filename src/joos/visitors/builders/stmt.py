class StmtBuilderMixin(object):
    ...
    def CreateIfThenElseStatement(self, klass, node):
        if node.rule.lhs == 'IfThenElseStatement':
            (exp, stmt_no_short_if, stmt) = self._resolve(
                node, 'Expression', 'StatementNoShortIf', 'Statement')
        else:
            (exp, stmt_no_short_if, stmt) = self._resolve(
                node, 'Expression', 'StatementNoShortIf', 'StatementNoShortIf')
    ...
    def CreateWhileStatement(self, klass, node):
        if node.rule.lhs == 'WhileStatement':
            ...
            (exp, stmt) = self._resolve(
                node, 'Expression', 'StatementNoShortIf')
        return klass(exp, stmt)

    def CreateForStatement(self, klass, node):
        if node.rule.lhs == 'ForStatement':
            (init, exp, update, stmt) = self._resolve(
                node, 'ForInit', 'Expression', 'ForUpdate', 'Statement')
        else:
            (init, exp, update, stmt) = self._resolve(
